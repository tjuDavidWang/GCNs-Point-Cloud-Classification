...
from deep_gcns_torch.gcn_lib.dense import BasicConv, GraphConv2d, ResDynBlock2d, DenseDynBlock2d, DilatedKnnGraph, PlainDynBlock2d


class DeepGCN(torch.nn.Module):
    def __init__(self, opt):
        super(DeepGCN, self).__init__()
        channels = opt.n_filters # 64
        k = opt.k # 9
        act = opt.act # relu
        norm = opt.norm # batch
        bias = opt.bias # True
        knn = 'matrix'  # implement knn using matrix multiplication
        epsilon = opt.epsilon # 0.2
        stochastic = opt.use_stochastic # True
        conv = opt.conv # edge
        c_growth = channels 
        emb_dims = opt.emb_dims # 1024
        self.n_blocks = opt.n_blocks # res
...
if __name__ == '__main__':
    ...
    feats = torch.rand((32, 3, 1024, 1), dtype=torch.float).to(args.device)
    num_neighbors = 20
