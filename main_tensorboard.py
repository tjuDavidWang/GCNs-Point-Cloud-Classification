...
def train(args, io, writer):
    ...
    for epoch in range(args.epochs):
        ...
        writer.add_scalar('train_avg_acc', train_avg_acc, epoch)
        ...
        writer.add_scalar('test_avg_acc', avg_per_class_acc, epoch)
...
def test(args, io, writer):
    ...
    writer.add_text('Test', f'test acc: {test_acc}, test avg acc: {avg_per_class_acc}', 0)
...
if __name__ == "__main__":
    ...
    if not args.eval:
        metrics = train(args, io, writer)
    else:
        metrics = test(args, io, writer)

    writer.close()

    # Save final results to config.log
    with open('checkpoints/' + args.exp_name + '/config.log', 'w') as f:
        f.write(str(args) + '\n')
